...
#   min_sup
#   inset(list[list])
# is_apriori(list list[set])
# 
# generate_Ck(list[list], int)
# 
# generate_Lk(inset, list[list], int, dic{})
# 
# 
    ...
    for i in range(len_l):
        for j in range(i+1, len_l):
            ...
            if l1[0:k-2] == l2[0:k-2]:
                ...
                if len(Ck_item) == k and is_apriori(Ck_item, Lksub):
                    Ck.append(Ck_item)
            l1, l2 = [], []
...
if __name__ == "__main__":
    #argv = sys.argv
    with open('sample_in.txt','r') as f:  # read sample input, just for example
        lines = f.readlines()
    ...
    L1 = set_minsup(min_support, sorted_wdcnt)
    #L1 done
    #generate Lk
